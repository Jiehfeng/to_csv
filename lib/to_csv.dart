...
Future myCSV(
    ...
    bool sharing =
        false, // Flag to decide whether to share the file after creation.
    String? fileName, // Optional custom file name.
    String? fileTimeStamp, // Optional timestamp for the file name.
    Map<int, int>?
        emptyRowsConfig, // Map to insert empty rows after specific rows.
    bool removeDuplicates =
        false, // Flag to remove duplicate values in the CSV.
    ...
    int?
        transposeAfterRow, // Row index after which the data will be transposed.
    bool ascendSortAfterTpOnFirstValue = false}) async {
  if (kDebugMode) {
  ...
  if (setHeadersInFirstRow) {
    ...
    headerAndDataList.addAll(
      listOfListOfStrings
          .map((row) => includeNoRow ? row : row.sublist(1))
          .toList(),
    );
  } else {
  ...
  if (transposeAfterRow != null &&
      transposeAfterRow < headerAndDataList.length) {
    List<List<String>> preTranspose =
        headerAndDataList.sublist(0, transposeAfterRow);
    List<List<String>> toTranspose =
        headerAndDataList.sublist(transposeAfterRow);
    ...
    if (ascendSortAfterTpOnFirstValue == true) {
      for (int j = transposeAfterRow + 1;
          j < headerAndDataList.length - 1;
          j++) {
        for (int i = transposeAfterRow + 1;
            i < headerAndDataList.length - 1;
            i++) {
